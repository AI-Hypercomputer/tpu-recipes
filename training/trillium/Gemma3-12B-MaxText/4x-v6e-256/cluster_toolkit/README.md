# Pretrain Gemma3-12B workload on Trillium GKE clusters with Cluster Toolkit

This recipe outlines the steps for running a Gemma3-12B
...
-   Precision: bfloat16
-   Chips: 1024 (4 slices of v6e-256, 16x16 topology, 64 hosts per slice)

The MaxText arguments and XLA flags in `run_recipe.sh` are expanded inline
so the script does not depend on the MaxText `benchmark_runner`. They are
the single-slice `gemma3_12b_32768_v6e256` model configuration from
[maxtext_trillium_model_configs.py](https://github.com/AI-Hypercomputer/maxtext/blob/tpu-recipes-v0.1.5/benchmarks/maxtext_trillium_model_configs.py)
...
`gemma3_12b_32768_4x_v6e256` configuration used by the XPK recipe in
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the Gemma3-12B workload.


## Test environment

This recipe is optimized for 4 v6e-256 slices (16x16 topology, 64 `ct6e-standard-4t` hosts per slice).
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-gemma3-12b-$(date +%H%M)` by default.
...
region, zone, `num_slices: 4`, `machine_type: ct6e-standard-4t`,
...
### Run Gemma3-12B Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the Gemma3-12B pretraining workload.
...
-   **MaxText Workload Overrides:** The `MAXTEXT_ARGS` variable holds the
    arguments passed to the `python3 -m maxtext.trainers.pre_train.train` command. This includes
