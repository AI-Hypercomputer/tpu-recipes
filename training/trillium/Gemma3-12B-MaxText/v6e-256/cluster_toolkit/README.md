...
-   Precision: bfloat16
-   Chips: 256 (v6e-256, 16x16 topology, 64 hosts)

The MaxText arguments and XLA flags in `run_recipe.sh` match the
`gemma3_12b_32768_v6e256` model configuration from
...
(profiler settings and the unused `dataset_path` omitted), expanded inline
so the script does not depend on the MaxText `benchmark_runner`.
...
This recipe is optimized for v6e-256 (16x16 topology, 64 `ct6e-standard-4t` hosts).
