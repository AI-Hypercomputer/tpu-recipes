# Pretrain Llama2-70B workload on Trillium GKE clusters with Cluster Toolkit

This recipe outlines the steps for running a Llama2-70B
...
The MaxText arguments and XLA flags in `run_recipe.sh` are the `llama2_70b_4096_sc`
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the Llama2-70B workload.
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-llama2-70b-$(date +%H%M)` by default.
...
### Run Llama2-70B Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the Llama2-70B pretraining workload.
