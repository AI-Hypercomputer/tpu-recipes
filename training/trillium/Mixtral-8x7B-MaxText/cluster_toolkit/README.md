# Pretrain Mixtral-8x7B workload on Trillium GKE clusters with Cluster Toolkit
...
-   Chips: 256 per slice (v6e-256, 16x16 topology, 64 hosts per slice);
    ...
    published for 1, 2 or 4 slices

The MaxText arguments and XLA flags in `run_recipe.sh` are the `mixtral_8x7b_dropped`
...
-   **Number of slices:** `NUM_SLICES` (default `1`) sets `--num-slices`.
    The recipe was published for 1, 2 or 4 v6e-256 slices. Your cluster
