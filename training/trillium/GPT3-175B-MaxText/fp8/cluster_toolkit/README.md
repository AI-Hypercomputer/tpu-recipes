# Pretrain GPT3-175B workload on Trillium GKE clusters with Cluster Toolkit
...
-   Precision: bfloat16 with AQT int8 quantization (`quantization=int8`)
-   Chips: 256 (v6e-256, 16x16 topology, 64 `ct6e-standard-4t` hosts)

The MaxText arguments and XLA flags in `run_recipe.sh` are the `gpt_3_175b`
...
> **Note:** Despite the `fp8` directory name, the `gpt_3_175b` configuration
> this recipe was defined by uses `quantization=int8`, which is kept here
> unchanged.
...
This recipe targets v6e-256 (16x16 topology, 64 `ct6e-standard-4t` hosts).
...
region, zone, `num_slices: 1`, `machine_type: ct6e-standard-4t`,
...
-   Maxtext version: `e7292a3a572792a0d797fc8977b21d0f255729f1`
-   Jax version: 0.4.35
-   Libtpu version: libtpu-nightly `0.1.dev20241028` (installed when the
    container starts)
...
cd maxtext
git checkout e7292a3a572792a0d797fc8977b21d0f255729f1

# Build and upload the docker image (pushed to gcr.io/<gcloud project>/${CLOUD_IMAGE_NAME}:latest)
bash docker_build_dependency_image.sh DEVICE=tpu MODE=stable JAX_VERSION=0.4.35
...
-   **MaxText Workload Overrides:** The `MAXTEXT_ARGS` variable holds the
    arguments passed to the `python3 MaxText/train.py` command. This includes
