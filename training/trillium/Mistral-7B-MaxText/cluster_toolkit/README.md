# Pretrain Mistral-7B workload on Trillium GKE clusters with Cluster Toolkit

This recipe outlines the steps for running a Mistral-7B
...
-   Precision: bfloat16
-   Chips: 8 (v6e-8, 2x4 topology, single host)

The MaxText arguments and XLA flags in `run_recipe.sh` are the `mistral_7b`
...
at MaxText `tpu-recipes-v0.1.2`, expanded inline so the script does not depend
on the MaxText `benchmark_runner`.
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the Mistral-7B workload.


## Test environment

This recipe is optimized for and tested with v6e-8 (2x4 topology, one
`ct6e-standard-8t` host).
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-mistral-7b-$(date +%H%M)` by default.
...
region, zone, `num_slices: 1`, `machine_type: ct6e-standard-8t`,
`tpu_topology: 2x4` and your reservation (see the example README), then deploy
...
### Run Mistral-7B Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the Mistral-7B pretraining workload.
