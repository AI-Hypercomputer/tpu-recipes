# Pretrain gpt3-175b workload on TPU v5p GKE clusters with Cluster Toolkit

This recipe outlines the steps for running a gpt3-175b
[MaxText](https://github.com/AI-Hypercomputer/maxtext) pretraining workload on
[TPU v5p GKE clusters](https://cloud.google.com/kubernetes-engine) by using
...
in the [XPK instructions](../README.md). To run on one of the larger documented
...
| TPU type  | Topology | `per_device_batch_size` | `remat_policy`          | `ici_data_parallelism` | `ici_fsdp_parallelism` | `ici_tensor_parallelism` |
| --------- | -------- | ----------------------- | ----------------------- | ---------------------- | ---------------------- | ------------------------ |
| v5p-1024  | 8x8x8    | 4                       | `full`                  | 1                      | 64                     | 8                        |
| v5p-2048  | 8x8x16   | 2                       | `save_dot_except_mlpwi` | 8                      | 16                     | 8                        |
| v5p-3072  | 8x12x16  | 2                       | `save_dot_except_mlpwi` | 12                     | 16                     | 8                        |
| v5p-4096  | 4x8x64   | 1                       | `minimal`               | 4                      | 64                     | 8                        |
| v5p-8192  | 8x16x32  | 1                       | `minimal`               | 16                     | 32                     | 8                        |
| v5p-12288 | 8x16x48  | 1                       | `minimal`               | 16                     | 48                     | 8                        |
...
-   **GCP Project Setup:** Ensure you have a GCP project with billing enabled
    and sufficient TPU v5p quota or a TPU v5p reservation.
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the gpt3-175b workload.


## Test environment

This recipe is configured for v5p-1024 (8x8x8 topology).
...
-   **GKE cluster** To create your GKE cluster, refer to the
    ...
    and the [Cluster Toolkit GKE TPU v5p example](https://github.com/GoogleCloudPlatform/cluster-toolkit/tree/main/examples/gke-tpu-v5p).
...
-   `WORKLOAD_IMAGE`: The Docker image for the workload. This is set in
    `run_recipe.sh` to
    `${CONTAINER_REGISTRY}/${PROJECT_ID}/${USER}-gpt3-175b-runner` by
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-gpt3-175b-$(date +%H%M)` by default.
...
clusters with Cloud TPU node pools. For detailed deployment instructions and configuration options for TPU v5p,
refer to the [Cluster Toolkit GKE TPU v5p example](https://github.com/GoogleCloudPlatform/cluster-toolkit/tree/main/examples/gke-tpu-v5p).
...
Deploy the TPU v5p blueprint to provision the GKE infrastructure using
`gcluster deploy`. Before deploying, edit
`examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml` to set `authorized_cidr`
and your reservation name under `reservation_affinity.specific_reservations`
(or choose one of the other consumption models documented in that file):
...
./gcluster deploy -d examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml \
  ...
  --vars="project_id=${PROJECT_ID},deployment_name=${CLUSTER_NAME},region=${REGION},zone=${ZONE},num_slices=1,machine_type=ct5p-hightpu-4t,tpu_topology=8x8x8"
```
...
### Run gpt3-175b Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the gpt3-175b pretraining workload.
