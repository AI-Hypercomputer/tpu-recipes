# Pretrain llama2-7b workload on TPU v5p GKE clusters with Cluster Toolkit

This recipe outlines the steps for running a llama2-7b
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the llama2-7b workload.


## Test environment

This recipe is configured for v5p-512 (4x8x8 topology).
...
-   `WORKLOAD_IMAGE`: The Docker image for the workload. This is set in
    `run_recipe.sh` to
    `${CONTAINER_REGISTRY}/${PROJECT_ID}/${USER}-llama2-7b-runner` by
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-llama2-7b-$(date +%H%M)` by default.
-   `ACCELERATOR_TYPE`: The TPU type (e.g., `v5p-512`, topology `4x8x8`). See topologies
...
./gcluster deploy -d examples/gke-tpu-v5p/gke-tpu-v5p-deployment.yaml \
  ...
  --vars="project_id=${PROJECT_ID},deployment_name=${CLUSTER_NAME},region=${REGION},zone=${ZONE},num_slices=1,machine_type=ct5p-hightpu-4t,tpu_topology=4x8x8"
```
...
### Run llama2-7b Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the llama2-7b pretraining workload.
