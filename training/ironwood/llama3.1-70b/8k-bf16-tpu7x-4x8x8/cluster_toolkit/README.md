# Ironwood (Cloud TPU v7x) pretrain llama3_1_70b workload on GKE clusters with Cluster Toolkit (gcluster)

This recipe outlines the steps for running a llama3_1_70b
...
-   **Pretraining job configuration and deployment** -
    ...
    resource, which manages the execution of the llama3_1_70b workload.
...
-   `WORKLOAD_IMAGE`: The Docker image for the workload. This is set in
    `run_recipe.sh` to
    `${CONTAINER_REGISTRY}/${PROJECT_ID}/${USER}-llama3_1_70b-runner` by
...
-   `WORKLOAD_NAME`: A unique name for your workload (maximum 28 characters).
    ...
    `${USER}-l70b-bf16-$(date +%H%M)` by default.
...
### Run llama3_1_70b Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the llama3_1_70b pretraining workload.
