# Pretrain gemma4-31b workload on Ironwood GKE clusters with Cluster Toolkit
...
-   `WORKLOAD_NAME`: A unique name for your workload. This defaults to
    `${CLEAN_USER}-gemma4-31b-$(date +%H%M%S)` in `run_recipe.sh`, where
    `CLEAN_USER` is the sanitized username.
...
python3 -m venv ${HOME}/.local/bin/venv-docker
...
cd tpu-recipes/training/ironwood/gemma4-31b/8k-bf16-tpu7x-4x4x4/cluster_toolkit
```
...
# List pods to find the specific name (e.g., gemma4-31b-0-0-xxxx)
