# Pretrain wan 2.2 workload on Ironwood GKE clusters with XPK

<!-- disableFinding(LINK_ID) -->

This recipe outlines the steps for running a wan 2.2
...
Tested and verified on `tpu7x-2x4x4` (64 TensorCores / 32 chips / 8 hosts):
* **Steady-State Step Time**: `13.57 s`
* **Per-Device Throughput**: `384.12 TFLOP/s/device`
* **Model FLOP Utilization (MFU)**: **`40.18%`** (against 956.4 BF16 Peak)
...
-   **Pretraining job configuration and deployment** - XPK is used to configure
    ...
    resource, which manages the execution of the wan 2.2 workload.
...
This recipe is optimized for and tested with tpu7x-2x4x4.
...
-   `WORKLOAD_IMAGE`: The Docker image for the workload. This is set in
    `run_recipe.sh` to
    `${CONTAINER_REGISTRY}/${PROJECT_ID}/${USER}-wan22-runner` by
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `${USER}-wan22-$(date +%H%M)` by default.
-   `GKE_VERSION`: The GKE version, `1.34.0-gke.2201000` or later.
-   `ACCELERATOR_TYPE`: The TPU type (e.g., `tpu7x-2x4x4`). See topologies
...
Before training, prepare the video training dataset in TFRecord format:
...
export HF_DATASET_DIR=/mnt/disks/external_disk/wan2.2_dataset/
export TFRECORDS_DATASET_DIR=/mnt/disks/external_disk/wan2.2_tfr_dataset
export HF_HUB_CACHE=/mnt/disks/external_disk/maxdiffusion_hf_cache/
export DATASET_DIR=${BASE_OUTPUT_DIR}/wan2.2-t2v-tfrecords

# Preprocess and convert dataset to TFRecord format with latents caching
...
# Upload preprocessed TFRecords to GCS
...
cd tpu-recipes/training/ironwood/wan2.2-t2v-14b/bf16-tpu7x-2x4x4/xpk
```

### Run wan 2.2 Pretraining Workload

The `run_recipe.sh` script contains all the necessary environment variables and
configurations to launch the wan 2.2 pretraining workload.
...
-   **MaxDiffusion Workload Overrides:** The `MAXDIFFUSION_ARGS` variable holds
    ...
    settings:
    - `attention=ulysses_ring_custom`: Enables custom Pallas splash attention backward kernel.
    - `flash_block_sizes`: Configures 3-slot aliased `dQ` reduction (`dq_reduction_steps=3`) and inner VPU sub-tiling (`bkv_compute_in=256`).
    - `ulysses_shards=4, ulysses_attention_chunks=2`: Enables chunked Ulysses context parallelism.
    - `remat_policy=CUSTOM`: Checkpoints `attn_output` to eliminate duplicate forward rematerialization.
...
# List pods to find the specific name (e.g., wan22-0-0-xxxx)
