...
# Set up and Activate Python 3.13 virtual environment for Docker build
uv venv --seed ~/.local/bin/venv-docker --python 3.13 --clear
source ~/.local/bin/venv-docker/bin/activate
pip install --upgrade pip

# Make sure you're running on a Virtual Environment with python 3.13
[[ "$(python3 -c 'import sys; print(f"{sys.version_info.major}.{sys.version_info.minor}")' 2>/dev/null)" == "3.13" ]] || { >&2 echo "Error: Python version must be 3.13."; false; }
