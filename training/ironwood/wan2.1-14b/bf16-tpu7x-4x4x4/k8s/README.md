# Pretrain wan2.1-14b workload on Ironwood GKE clusters with Kubernetes JobSet
...
-   **Container Image:** A pre-built container image (such as
    `gcr.io/my-project/my-maxtext-runner:latest`) containing the MaxDiffusion
...
-   **Pretraining job configuration and deployment** - A Kubernetes manifest
    ...
    resource, which manages the execution of the MaxDiffusion pretraining workload.
...
export WORKLOAD_NAME="$(printf "%.26s" "${USER//_/-}-wan2.1-14b")-$(date +%Y%m%d-%H%M)"
```

### 2. Run wan2.1-14b Pretraining Workload
