# Run vLLM qwen3_coder_480b_a35b_instruct on TPU Trillium (v6e) on GKE

This recipe covers running a vLLM inference workload on TPU Trillium (v6e) on
GKE.

## Create the GKE Cluster
Create your TPU Trillium (v6e) cluster using [XPK](https://github.com/AI-Hypercomputer/xpk).
The next sections assume you have created a cluster with TPU nodes.
...
Before running the benchmark, please update the placeholders in `values.yaml`:
- `gcp_service_account`: Set to your GCP service account.
- `--model`: Update the bucket path for your model in `values.yaml` (currently `gs://<YOUR_BUCKET>/Qwen3-Coder-480B-A35B-Instruct`).
