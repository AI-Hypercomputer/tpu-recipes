# Inference Wan-AI/Wan2.1-T2V-14B-Diffusers workload on Ironwood GKE clusters with XPK.
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    ...
    `export WORKLOAD_NAME="$(printf "%.26s" "${USER//_/-}-wan2-1-t2v")-$(date +%Y%m%d-%H%M)"`
...
cd tpu-recipes/inference/ironwood/MaxDiffusion/Wan2.x/Wan2.1-T2V
```
