...
Before running the benchmark, please ensure you update `values.yaml` with your specific values for the following placeholders:
- `<YOUR_GCP_SERVICE_ACCOUNT>`: The GCP service account with necessary permissions (e.g. `gcp_service_account` field).
- `<YOUR_MODEL_PATH>`: The path to the downloaded model checkpoint or a GCS bucket (e.g. `model` arg in `server.bash_command`).
